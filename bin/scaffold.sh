...
# - boostraps accounts for terraform best practices
# - handles remote state
# - uses consistent .tfvars files for each environment
# - Lighter weight version of what terragrunt does
...
Usage: ${0} \\
  ...
  -p/--project       [project] \\
  -r/--region        [region] \\
  -u/--unique        [true] \\ future feature - extra noise for bucket names
  -- \\
...
bucket_name (optional override):
 Defaults to: "[account-alias]-scaffold" or "[project]-[account-id]-scaffold"
 - myproject-terraform
 - terraform-yourproject
 - my-first-scaffold-project
...
environment: (used in resource names and deriving environment tfvars file name)
 - dev
 - test
 - prod
 - management
...
ARGS=$(getopt \
         -o hva:b:c:e:g:i:p:r: \
         -l "help,version,bootstrap,action:,bucket-name:,build-id:,component:,environment:,group:,project:,region:" \
         -n "${0}" \
         -- \
         "$@");

# Check for exit code of getopts call to see if it choked
if [ $? -ne 0 ]; then
  usage;
...
iam_caller_id="$(aws sts get-caller-identity --query 'Arn' --output text)";
if [ -n "${iam_caller_id}" ]; then
  echo -e "AWS Credentials Found. Using ARN '${iam_caller_id}'";
else
  error_and_die "No AWS Credentials Found. \"aws sts get-caller-identity --query 'Arn' --output text\" responded with ARN '${iam_caller_id}'";
fi;
...
# Query AWS account alias to use unless overridden 
aws_account_alias="$(aws iam list-account-aliases --output text  --query 'AccountAliases')";
if [ -n "${aws_account_alias}" ]; then
  echo -e "AWS Account Alias: ${aws_account_alias}";
fi;

# Validate S3 bucket. Set default if undefined, derive logbucket name
if [ -n "${bucket_name}" ]; then
  readonly backend_name="${bucket_name}"
  readonly logbucket_name="${bucket_name}-logs"
  echo -e "Using S3 bucket s3://${backend_name}";
elif [ -n "${aws_account_alias}" ]; then
  readonly backend_name="${aws_account_alias}-scaffold"
  readonly logbucket_name="${aws_account_alias}-logs"
    echo -e "No bucket prefix specified. Using S3 bucket s3://${backend_name}";
else
  readonly backend_name="${project}-${aws_account_id}-scaffold"
  readonly logbucket_name="${project}-${aws_account_id}-logs"
  echo -e "No bucket name specified. Using S3 bucket s3://${backend_name}";
fi;
...
if [ "${bootstrap}" == "true" ]; then
  ...
  tf_var_params+=" -var backend_name=${backend_name}";
  tf_var_params+=" -var logbucket_name=${logbucket_name}";
  ...
  # Check for presence of a local variables file, and use it if readable
  readonly local_vars_file_name="local.tfvars";
  readonly local_vars_file_path="${component_path}/${local_vars_file_name}"
  ...
  [ -f "${local_vars_file_path}" ] && tf_var_file_paths+=("${local_vars_file_path}");
...
###################################################################";
  ...
  # Build up the tfvars arguments for terraform command line then add in the backend name at the end
...
readonly backend_config="terraform {
  backend \"s3\" {
    ...
    bucket = \"${backend_name}\"
    key    = \"${backend_key}\"
    dynamodb_table = \"${backend_name}-lock\"
  }
}";
...
if [ "${bootstrap}" == "true" ]; then
  ...
  echo "Checking to see s3://${backend_name}/${backend_prefix}/${backend_filename} exists"
  aws s3 ls s3://${backend_name}/${backend_prefix}/${backend_filename} >/dev/null 2>&1;
...
if [ "${bootstrapped}" == "true" ]; then
  ...
  echo "Setting up S3 remote state from s3://${backend_name}/${backend_key}";
...
case "${action}" in
  'plan')
    ...
    if [ -n "${build_id}" ]; then
      aws s3 cp build/${plan_file_name} s3://${backend_name}/${plan_file_remote_key} \
        || error_and_die "Plan file upload to S3 failed (s3://${backend_name}/${plan_file_remote_key})";
    fi;
  ...
  'apply'|'destroy')
    ...
    if [ -n "${build_id}" ]; then
      ...
      aws s3 cp s3://${backend_name}/${plan_file_remote_key} build/${plan_file_name} \
        || error_and_die "Plan file download from S3 failed (s3://${backend_name}/${plan_file_remote_key})";
